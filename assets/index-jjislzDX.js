(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))s(n);new MutationObserver(n=>{for(const i of n)if(i.type==="childList")for(const r of i.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&s(r)}).observe(document,{childList:!0,subtree:!0});function t(n){const i={};return n.integrity&&(i.integrity=n.integrity),n.referrerPolicy&&(i.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?i.credentials="include":n.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function s(n){if(n.ep)return;n.ep=!0;const i=t(n);fetch(n.href,i)}})();const b="/fifteen-puzzle/assets/solved-CX2STtVs.opus";class y{audio;constructor(){this.audio=new Audio(b)}playSuccess(){this.audio.currentTime=0,this.audio.play()}}const a=4,l=16,c=l;class w{animationId=null;state;updateCallback;constructor(e,t){this.state=e,this.updateCallback=t}start(){this.state.startTime===0&&(this.state.ticking=!0,this.state.startTime=performance.now(),this.update())}stop(){this.state.ticking=!1,this.animationId&&(cancelAnimationFrame(this.animationId),this.animationId=null)}update=()=>{this.state.ticking&&(this.state.endTime=performance.now(),this.state.timeElapsed=(this.state.endTime-this.state.startTime)/1e3,this.state.minutes=~~(this.state.timeElapsed/60),this.state.seconds=~~(this.state.timeElapsed-this.state.minutes*60),this.updateCallback(),this.animationId=requestAnimationFrame(this.update))}}const h=(o,e)=>{const t=o.toString().padStart(2,"0"),s=e.toString().padStart(2,"0");return`${t}:${s}`},v=o=>{const e=[...o];for(let t=e.length-1;t>0;t--){const s=~~(Math.random()*(t+1));[e[t],e[s]]=[e[s],e[t]]}return e},E=o=>{let e=0;for(let n=0;n<c-1;n++)for(let i=n+1;i<c;i++)o[n]!==l&&o[i]!==l&&o[n]>o[i]&&e++;const t=~~(o.indexOf(l)/a);return(a-t)%2===0?e%2===1:e%2===0},I=o=>{const e=~~(o/a),t=o%a,s=[];return e>0&&s.push(o-a),t<a-1&&s.push(o+1),e<a-1&&s.push(o+a),t>0&&s.push(o-1),s},S=()=>{let o;do o=v(Array.from({length:c},(e,t)=>t+1));while(!E(o));return o};class k{state;timer;audioManager;renderCallback;constructor(e){this.state={orderedIndex:[],elementValue:[],show:!1,ticking:!1,startTime:0,endTime:0,timeElapsed:0,minutes:0,seconds:0,moves:0},this.renderCallback=e,this.timer=new w(this.state,this.renderCallback),this.audioManager=new y}initialize(){this.state.orderedIndex=Array.from({length:c},(e,t)=>t+1),this.reset()}reset(){this.state.elementValue=S(),this.state.startTime=0,this.state.endTime=0,this.state.timeElapsed=0,this.state.moves=0,this.state.minutes=0,this.state.seconds=0,this.timer.stop(),this.renderCallback()}moveTile(e){const t=this.state.elementValue.indexOf(e),s=this.state.elementValue.indexOf(l);I(s).includes(t)&&([this.state.elementValue[t],this.state.elementValue[s]]=[this.state.elementValue[s],this.state.elementValue[t]],this.state.moves++,this.timer.start(),this.renderCallback(),this.checkWin())}isSolved(){return this.state.elementValue.every((e,t)=>e===this.state.orderedIndex[t])}handleKeyPress(e){const t={ArrowUp:a,ArrowDown:-a,ArrowLeft:1,ArrowRight:-1};if((e.code in t||e.code==="KeyR")&&e.preventDefault(),e.code==="KeyR"){this.reset();return}const s=t[e.code];if(s){const n=this.state.elementValue.indexOf(l),i=n+s;if(i>=0&&i<c){const r=~~(n/a),d=~~(i/a);if(Math.abs(s)===1&&r!==d)return;this.moveTile(this.state.elementValue[i])}}}checkWin(){this.isSolved()?(this.state.show=!0,this.timer.stop(),this.audioManager.playSuccess(),this.renderCallback()):this.state.show=!1}}class T{game;prevElementValue=[];constructor(e){this.game=e}createTile(e){const t=document.createElement("div"),s=e===l,n=e===this.game.state.orderedIndex[this.game.state.elementValue.indexOf(e)];return t.id=e.toString(),t.className=`
    ...
    `}render(){const e=document.getElementById("app");e&&(e.innerHTML=this.createHtml(),this.initTheme(),window.addEventListener("keydown",this.preventInteraction,!0),document.getElementById("shuffle-btn")?.addEventListener("click",()=>{this.game.reset()}),document.getElementById("modal-shuffle-btn")?.addEventListener("click",()=>{this.game.state.show=!1,this.updateDisplay(),this.game.reset()}),document.getElementById("theme-btn")?.addEventListener("click",()=>{const t=document.documentElement,s=t.getAttribute("data-theme")==="alt"?"base":"alt";s==="alt"?t.setAttribute("data-theme","alt"):t.removeAttribute("data-theme"),localStorage.setItem("theme",s)}),document.getElementById("controls-btn")?.addEventListener("click",()=>{this.openControls()}),document.getElementById("controls-back-btn")?.addEventListener("click",()=>{this.closeControls()}),this.update())}update(){this.renderGrid(),this.updateDisplay()}arraysEqual(e,t){if(e===t)return!0;if(e.length!==t.length)return!1;for(let s=0;s<e.length;s++)if(e[s]!==t[s])return!1;return!0}renderGrid(){const e=document.querySelector(".grid");if(!e)return;const t=this.game.state.elementValue;if(this.arraysEqual(this.prevElementValue,t))return;const s=document.createDocumentFragment();this.game.state.elementValue.forEach(n=>{s.appendChild(this.createTile(n))}),e.innerHTML="",e.appendChild(s),this.prevElementValue=[...t]}updateDisplay(){const e=h(this.game.state.minutes,this.game.state.seconds),t=this.game.state.moves.toString(),s=document.querySelector('figure[aria-label="tracker"] time:first-child p'),n=document.querySelector('figure[aria-label="tracker"] time:last-child p');s&&(s.textContent=e),n&&(n.textContent=t);const i=document.querySelector('figure[aria-label="modal-tracker"] time:first-child p'),r=document.querySelector('figure[aria-label="modal-tracker"] time:last-child p');i&&(i.textContent=e),r&&(r.textContent=t);const d=document.getElementById("modal");d&&(this.game.state.show?d.classList.remove("hidden"):d.classList.add("hidden"))}isControlsOpen(){const e=document.getElementById("controls");return!!e&&!e.classList.contains("hidden")}isSolvedOpen(){const e=document.getElementById("modal");return!!e&&!e.classList.contains("hidden")}preventInteraction=e=>{(this.isControlsOpen()||this.isSolvedOpen())&&(e.preventDefault(),e.stopImmediatePropagation())};openControls(){const e=document.getElementById("controls");e&&e.classList.remove("hidden")}closeControls(){const e=document.getElementById("controls");e&&e.classList.add("hidden")}initTheme(){const e=document.documentElement;localStorage.getItem("theme")==="alt"?e.setAttribute("data-theme","alt"):e.removeAttribute("data-theme")}}/**
